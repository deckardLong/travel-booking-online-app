...
const Home = ({navigation}) => {
    ...
    const fetchTabItems = async () => {
        ...
        try {
            ...
            if (activeTab === 'transports' && vehicleType !== '') {
                queryParts.push(`vehicle_type=${vehicleType}`);
            }

            let queryString = queryParts.join('&');
            let url = `${endpoints[activeTab]}?${queryString}`;

            // 🛑 TRẠM GÁC SỐ 1: Kiểm tra URL trước khi gửi đi
            console.log("-----------------------------------------");
            console.log(`[DEBUG API] Đang chuẩn bị gọi: ${url}`);

            let res = await apis.get(url);

            const newItems = res.data.results || [];

            // 🛑 TRẠM GÁC SỐ 2: Kiểm tra dữ liệu Server trả về
            console.log(`[DEBUG DATA] Server trả về ${newItems.length} kết quả.`);
            if (newItems.length > 0 && activeTab === 'transports') {
                // In thử loại phương tiện của kết quả đầu tiên xem có đúng không
                console.log(`[DEBUG ITEM 1] Loại PT của item đầu tiên: ${newItems[0].vehicle_type}`);
            }
            console.log("-----------------------------------------");
            ...
            // 🛑 TRẠM GÁC SỐ 3: Bắt lỗi chi tiết
            console.log("-----------------------------------------");
            console.error(`[DEBUG ERROR] Lỗi khi gọi API ${activeTab}:`);
            
            if (ex.response) {
                // Server có phản hồi nhưng báo lỗi (vd: 400, 404, 500)
                console.error("- Status:", ex.response.status);
                console.error("- Data:", ex.response.data);
            } else if (ex.request) {
                // Gửi đi được nhưng không nhận được phản hồi (vd: Sập server, mất mạng)
                console.error("- Không nhận được phản hồi từ Server:", ex.message);
            } else {
                // Lỗi ngay trong code React Native (vd: sai cú pháp)
                console.error("- Lỗi cú pháp/Logic:", ex.message);
            }
            console.log("-----------------------------------------");
    ...
    useEffect(() => {
        setPage(1);
        setHasNextPage(true);
    }, [activeTab, JSON.stringify(filterParams), searchQuery, vehicleType]);
    ...
    }, [activeTab, JSON.stringify(filterParams), searchQuery, page, vehicleType]);
            ...
            <Card.Content style={styles.cardContent}>
                ...
                <View style={styles.itemFooter}>
                    ...
                    <Text style={styles.itemPrice}>
                        {formatPrice(item.price) ? `${formatPrice(item.price).toLocaleString()} VNĐ`: 'Liên hệ'}
                    </Text>
                </View>
            </Card.Content>
            <Card.Actions>
        ...
        <View style={styles.container}>
                ...
                <View style={styles.headerTop}>
                    ...
                    <View style={styles.avatarBorder}>
                        {user?.avatar ? (
                            ...
                            <Avatar.Icon size={65} icon="account" color='#088395' style={{backgroundColor: '#fff'}} />
                        )}
                    </View>
                </View>
            ...
            <View style={styles.tabContainer}>
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                    {tabs.map(tab => (
                        <Chip
                            ...
                            selectedColor='#fff'
                            backgroundColor={activeTab === tab.id ? '#088395' : '#E2E8F0'}
                            textStyle={{
                ...
                {activeTab === 'transports' && (
                    <View style={{ flexDirection: 'row', paddingHorizontal: 15, paddingBottom: 10, gap: 10 }}>
                        <Chip 
                            icon="bus-side" 
                            selected={vehicleType === '' || vehicleType === 'BUS'} 
                            onPress={() => setVehicleType(vehicleType === 'BUS' ? '' : 'BUS')}
                            style={{ backgroundColor: (vehicleType === '' || vehicleType === 'BUS') ? '#E0F2F1' : '#F1F5F9' }}
                            textStyle={{ color: (vehicleType === '' || vehicleType === 'BUS') ? '#088395' : '#64748B', fontWeight: 'bold' }}
                        >
                            Xe khách
                        </Chip>
                        <Chip 
                            icon="airplane" 
                            selected={vehicleType === 'FLIGHT'} 
                            onPress={() => setVehicleType(vehicleType === 'FLIGHT' ? '' : 'FLIGHT')}
                            style={{ backgroundColor: vehicleType === 'FLIGHT' ? '#E0F2F1' : '#F1F5F9' }}
                            textStyle={{ color: vehicleType === 'FLIGHT' ? '#088395' : '#64748B', fontWeight: 'bold' }}
                        >
                            Máy bay
                        </Chip>
                        <Chip 
                            icon="ferry" 
                            selected={vehicleType === 'SHIP'} 
                            onPress={() => setVehicleType(vehicleType === 'SHIP' ? '' : 'SHIP')}
                            style={{ backgroundColor: vehicleType === 'SHIP' ? '#E0F2F1' : '#F1F5F9' }}
                            textStyle={{ color: vehicleType === 'SHIP' ? '#088395' : '#64748B', fontWeight: 'bold' }}
                        >
                            Tàu thủy
                        </Chip>
                    </View>
            ...
            {loading ? (
                <ActivityIndicator style={styles.loader} color='#088395'size="large" />
            ) : (
                <FlatList
                    data={data}
                    ...
                    ListEmptyComponent={
                        <Text style={styles.emptyText}>Không tìm thấy kết quả nào. Hãy thử tìm kiếm khác nhé!</Text> 
                    }
                    ListFooterComponent={loadingMore ? <ActivityIndicator size="large" color='#088395' style={{marginVertical: 20}} /> : null}
                />
            )}
            <FAB
                icon="compare"
                label={compareList.length > 0 ? `So sánh (${compareList.length})` : "So sánh"}
                color='#fff'
                style={{
                    position: 'absolute', 
                    right: 0,         
