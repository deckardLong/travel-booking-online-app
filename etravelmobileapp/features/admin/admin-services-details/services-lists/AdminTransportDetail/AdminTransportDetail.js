...
import styles, { PRIMARY_COLOR, SUCCESS_COLOR } from "./AdminTransportDetailStyles";
import apis, { endpoints } from "../../../../../configs/apis";

const AdminTransportDetail = ({ route, navigation }) => {
    ...
    const loadDetail = async () => {
        try {
            ...
            Alert.alert("Lỗi", "Không thể tải thông tin phương tiện. Có thể dịch vụ đã bị xóa.");
        } finally {
            setLoading(false);
        }
    };
    ...
    if (!transport) return (
        <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text>Dữ liệu phương tiện không tồn tại.</Text>
                ...
                <View style={styles.content}>
                    ...
                    <Surface style={styles.adminInfoBox} elevation={0}>
                        ...
                        <View style={styles.adminInfoRow}>
                            <Text style={styles.adminInfoLabel}>ID Phương tiện:</Text>
                            <Text style={styles.adminInfoValue}>#{transport.id}</Text>
                        </View>
                        <View style={styles.adminInfoRow}>
                            <Text style={styles.adminInfoLabel}>Hãng / Nhà xe:</Text>
                            <Text style={[styles.adminInfoValue, { color: PRIMARY_COLOR }]}>
                                {transport.provider_name || `ID: ${transport.provider}`}
                            </Text>
                        </View>
                        <View style={styles.adminInfoRow}>
                            <Text style={styles.adminInfoLabel}>Trạng thái:</Text>
                            <Text style={[styles.adminInfoValue, { color: transport.active ? SUCCESS_COLOR : '#EF4444' }]}>
                                {transport.active ? "Đang hoạt động" : "Đang ẩn"}
                            </Text>
                        </View>
                    </Surface>
                    ...
                    <View style={styles.highlightGrid}>
                        ...
                        <View style={styles.gridItem}>
                            ...
                            <Text style={styles.gridLabel}>Giờ chạy</Text>
                        </View>
                        <View style={styles.gridItem}>
                    ...
                    <RenderHtml
                        ...
                        source={{ html: transport.description || "<p>Chưa có thông tin mô tả.</p>" }}
